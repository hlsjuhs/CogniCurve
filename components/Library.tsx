...
export const Library: React.FC<LibraryProps> = ({ items, onUpdateItem, onImportItems, onDeleteItems }) => {
  ...
  const executeBatchDelete = () => {
      if (confirm(`确定要彻底删除选中的 ${selectedIds.size} 个条目吗？\n此操作不可恢复。`)) {
                  ...
                  {batchActionStep === 'none' ? (
                      <>
                        ...
                        <button 
                            ...
                            className="px-4 py-3 rounded-xl hover:bg-slate-800 transition-colors flex items-center justify-center gap-2 text-sm text-rose-400 hover:text-rose-300 hover:bg-rose-900/20"
                        >
